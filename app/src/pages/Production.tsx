/**
 * Production.tsx — Manufacturing Orders
 * Uses the same /api/production backend endpoint as Work Orders.
 * This page treats each Production record as a "Manufacturing Order".
 */
...
function ProgressBar({ pct }: { pct: number }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
      <div style={{ width: "96px", height: "6px", borderRadius: "999px", background: "var(--card-border)", overflow: "hidden" }}>
        <div style={{ width: `${pct}%`, height: "100%", background: pct === 100 ? "var(--success-text)" : pct > 50 ? "var(--accent)" : "var(--warning-text)", borderRadius: "999px", transition: "width 0.3s ease" }} />
      </div>
      <span style={{ fontSize: "11px", color: "var(--text)", minWidth: "28px" }}>{pct}%</span>
    </div>
  );
}
...
interface MOFormData {
  product: string; machine: string; status: string; priority: string;
  quantityPlanned: string; quantityProduced: string;
  plannedStartDate: string; plannedEndDate: string; notes: string;
}

const DEFAULT_FORM: MOFormData = {
...
function MOForm({
  initial = DEFAULT_FORM, products, machines, onSave, onCancel, onDelete, isEdit,
}: {
  initial?: MOFormData; products: ProductRecord[]; machines: MachineRecord[];
  onSave: (d: MOFormData) => Promise<string | null>; onCancel: () => void;
  onDelete?: () => void; isEdit?: boolean;
}) {
  const [form, setForm] = useState<MOFormData>(initial);
  const [errors, setErrors] = useState<Partial<MOFormData>>({});
  const [saving, setSaving] = useState(false);
  const [apiError, setApiError] = useState<string | null>(null);

  function set(field: keyof MOFormData, value: string) {
    setForm((f) => ({ ...f, [field]: value }));
    setErrors((e) => ({ ...e, [field]: undefined }));
  }

  function validate(): boolean {
    const e: Partial<MOFormData> = {};
  ...
  return (
    <form onSubmit={handleSubmit} noValidate>
      ...
      <FormActions
        onCancel={onCancel}
        submitLabel={isEdit ? "Update Order" : "Create Manufacturing Order"}
        loading={saving}
        destructive={isEdit}
        onDestruct={onDelete}
        destructLabel="Delete Order"
      />
    </form>
  );
}
...
export default function Production() {
  ...
  const filtered = records.filter((w) => {
    ...
    return w.workOrderNumber.toLowerCase().includes(q) || getProductName(w.product).toLowerCase().includes(q);
  });

  const active    = records.filter((w) => w.status === "in_progress").length;
  const scheduled = records.filter((w) => w.status === "planned").length;
  const completed = records.filter((w) => w.status === "completed").length;
  const onHold    = records.filter((w) => w.status === "on_hold").length;

  async function handleCreate(data: MOFormData): Promise<string | null> {
    const res = await workOrdersApi.create({
      product: data.product, machine: data.machine || undefined,
      status: data.status, priority: data.priority,
      quantityPlanned: Number(data.quantityPlanned),
      quantityProduced: Number(data.quantityProduced),
      plannedStartDate: data.plannedStartDate, plannedEndDate: data.plannedEndDate, notes: data.notes,
    });
  ...
  async function handleEdit(data: MOFormData): Promise<string | null> {
  ...
  function toForm(wo: WorkOrderRecord): MOFormData {
    ...
    return {
      ...
      plannedStartDate: wo.plannedStartDate?.slice(0, 10) ?? "",
      plannedEndDate:   wo.plannedEndDate?.slice(0, 10) ?? "",
      notes: wo.notes,
    };
  }

  const calcPct = (wo: WorkOrderRecord) =>
    wo.quantityPlanned === 0 ? 0 : Math.round((wo.quantityProduced / wo.quantityPlanned) * 100);

  return (
    <PageShell
      title="Manufacturing Orders"
      subtitle="Work orders, production lines, and manufacturing schedules."
      action={<ActionButton label="+ New Manufacturing Order" onClick={() => setShowCreate(true)} />}
    >
      <div style={{ display: "flex", flexWrap: "wrap", gap: "16px", marginBottom: "28px" }}>
        <KpiCard label="Active Orders"    value={String(active)}    sub="Currently running"   accent />
        <KpiCard label="Scheduled"        value={String(scheduled)} sub="Queued to start"            />
        <KpiCard label="Completed"        value={String(completed)} sub="Finished orders"            />
        <KpiCard label="On Hold"          value={String(onHold)}    sub="Awaiting resolution"        />
      </div>
      ...
      <SectionCard title="Manufacturing Orders" subtitle="All production work orders">
        {loading ? (
          <div style={{ padding: "40px", textAlign: "center", color: "var(--text)" }}>Loading…</div>
        ) : filtered.length === 0 ? (
          <div style={{ padding: "40px", textAlign: "center", color: "var(--text)" }}>No orders found.</div>
        ) : (
          <DataTable columns={["Order #", "Product", "Qty Plan/Done", "Priority", "Progress", "Status", "Due Date", ""]}>
            {filtered.map((wo, idx) => (
              <DataRow
                key={wo._id}
                index={idx}
                cells={[
                  <span style={{ fontFamily: "monospace", fontSize: "12px", color: "var(--accent)" }}>{wo.workOrderNumber}</span>,
                  getProductName(wo.product),
                  `${wo.quantityPlanned} / ${wo.quantityProduced}`,
                  <StatusPill label={PRIORITY_LABEL[wo.priority] ?? wo.priority} variant={PRIORITY_VARIANT[wo.priority] ?? "neutral"} />,
                  <ProgressBar pct={calcPct(wo)} />,
                  <StatusPill label={STATUS_LABEL[wo.status] ?? wo.status} variant={STATUS_VARIANT[wo.status] ?? "neutral"} />,
                  wo.plannedEndDate?.slice(0, 10) ?? "—",
                  <button
      ...
      <Modal open={showCreate} title="New Manufacturing Order" onClose={() => setShowCreate(false)} width={620}>
        <MOForm products={products} machines={machines} onSave={handleCreate} onCancel={() => setShowCreate(false)} />
      </Modal>

      <Modal open={!!editRecord} title={`Edit Order — ${editRecord?.workOrderNumber ?? ""}`} onClose={() => setEditRecord(null)} width={620}>
        {editRecord && (
          <MOForm
            initial={toForm(editRecord)}
            products={products} machines={machines}
            onSave={handleEdit} onCancel={() => setEditRecord(null)}
            onDelete={() => setDeleteTarget(editRecord)} isEdit
          />
        )}
      </Modal>
      ...
      <ConfirmDialog
        ...
        message={`Delete order ${deleteTarget?.workOrderNumber}? This cannot be undone.`}
